...
var (
	dfusePath      = flag.String("dfuse-path", "/opt/daos/bin/dfuse", "dfuse path")
	volumeBasePath = flag.String("volume-base-path", "/daos-tmp/.volumes", "volume base path")
...
func main() {
	...
	klog.Infof("Running Daos CSI driver sidecar mounter version %v", version)
	...
	mounter := sidecarmounter.New(*dfusePath)
	...
	for _, sp := range socketPathes {
		...
		go func(mc *sidecarmounter.MountConfig) {
			...
			if err != nil {
				errMsg := fmt.Sprintf("failed to mount daos container %q for volume %q: %v\n", mc.DaosContainerName, mc.VolumeName, err)
				klog.Errorf(errMsg)
				// if _, e := errWriter.Write([]byte(errMsg)); e != nil {
				// 	klog.Errorf("failed to write the error message %q: %v", errMsg, e)
				// }
			...
			if err = cmd.Start(); err != nil {
				errMsg := fmt.Sprintf("failed to start dfuse with error: %v\n", err)
				klog.Errorf(errMsg)
				// if _, e := errWriter.Write([]byte(errMsg)); e != nil {
				// 	klog.Errorf("failed to write the error message %q: %v", errMsg, e)
				// }
			...
			// Since the dfuse has taken over the file descriptor,
			// closing the file descriptor to avoid other process forking it.
			syscall.Close(mc.FileDescriptor)
			if err = cmd.Wait(); err != nil {
				errMsg := fmt.Sprintf("For volume [%v] exited with error: %v\n", mc.VolumeName, err)
				klog.Errorf(errMsg)
				// if _, e := errWriter.Write([]byte(errMsg)); e != nil {
				// 	klog.Errorf("failed to write the error message %q: %v", errMsg, e)
				// }
			} else {
				klog.Infof("For volume [%v] mount command exited normally.", mc.VolumeName)
			}
		}(mc)
	}
	...
	klog.Info("received SIGTERM signal, waiting for all the dfuse processes exit...")
	wg.Wait()
...
func prepareMountConfig(sp string) (*sidecarmounter.MountConfig, error) {
	...
	if mc.DaosContainerName == "" {
		return nil, fmt.Errorf("failed to fetch cotainer name from CSI driver")
	}
